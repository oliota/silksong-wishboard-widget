...
function Show-EditSettingsWindow {
        ...
        try {
            ...
            $listener.BeginGetContext({
                ...
                try {
                    ...
                    Save-Config
                    $calendarStatus.Dispatcher.Invoke([Action]{ $calendarStatus.Text = 'Google Calendar connected and saved.'; $calendarStatus.Foreground = [System.Windows.Media.Brushes]::LightGreen; $calendarImport.IsEnabled = $true })
                } catch {
                    $message = $_.Exception.Message
                    $calendarStatus.Dispatcher.Invoke([Action]{ $calendarStatus.Text = 'Authorization failed: ' + $message; $calendarStatus.Foreground = [System.Windows.Media.Brushes]::IndianRed; $calendarImport.IsEnabled = $true })
    ...
    $calendarConfirm.Add_Click({
        $path = Join-Path $base 'google-calendar-credentials.json'
        try {
            if (-not (Test-Path -LiteralPath $path)) { throw 'Credentials file not found in the project.' }
            $check = @(Get-Content -LiteralPath $path | ConvertFrom-Json)[0]
            if ($null -eq $check -or $null -eq $check.installed -or [string]::IsNullOrWhiteSpace([string]$check.installed.client_id) -or [string]::IsNullOrWhiteSpace([string]$check.installed.client_secret)) { throw 'Invalid credentials structure.' }
            $script:config.googleCalendar.configured = $true
            Save-Config
            $calendarStatus.Text = 'Connection configuration confirmed.'
            $calendarStatus.Foreground = [System.Windows.Media.Brushes]::LightGreen
        } catch { $calendarStatus.Text = 'Connection failed: ' + $_.Exception.Message; $calendarStatus.Foreground = [System.Windows.Media.Brushes]::IndianRed }
    }.GetNewClosure())
    $calendarTab.Content = New-SettingsContentFrame $calendarPanel
